...
public class CreateProjectCommandHandler implements ICommandHandler<CreateProjectCommand, Project> {
	...
	public Project handle(@Valid final CreateProjectCommand command) {
		...
		final Project existingProject = this.projectDAO.findByKey(project.getKey());
		...
		if (StringUtils.isEmpty(project.getStatusId())) {
			final ProjectStatus defaultStatus = this.statusDAO.getDefaultStatus();
		...
		this.revisionService.commit(project);
		...
		this.gate.dispatchEvent(eventNewProjectCreated);

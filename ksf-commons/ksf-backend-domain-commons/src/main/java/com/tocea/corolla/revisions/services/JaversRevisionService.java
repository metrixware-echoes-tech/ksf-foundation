...
public class JaversRevisionService implements IRevisionService {
    ...
    @Override
    public String commit(final Object obj) {
    ...
    public List<IChange> compare(final Object oldVersion, final Object currentVersion) {
        ...
        final Diff diff = this.javers.compare(oldVersion, currentVersion);
    ...
    public CdoSnapshot findCdoSnapshotByCommit(final Commit commit) {
        ...
        final List<CdoSnapshot> snapshots = this.javers.findSnapshots(QueryBuilder.byInstanceId(id, clazz).build());

        if (snapshots != null) {
            for (final CdoSnapshot snapshot : snapshots) {
                final String commitID = commitIDAsString(snapshot);
    ...
    private static String commitIDAsString(final CdoSnapshot snapshot) {
        return snapshot.getCommitId().valueAsNumber().toBigInteger().toString();
    }

    @Override
    public ICommit findCommitByID(final String objectID, final Class<?> objectClass, final String commitID) {

        final Collection<ICommit> history = this.getHistory(objectID, objectClass);
    ...
    @Override
    public List<ICommit> getHistory(final String id, final Class<?> clazz) {
    ...
    public Object getSnapshot(final ICommit commit) {
        ...
        final List<CdoSnapshot> history = this.javers.findSnapshots(QueryBuilder.byInstanceId(commit.getObjectID(), commit.getObjectClass()).build());
        ...
        try {
            ...
            snapshot = clazz.newInstance();
            for (int i = 1, historySize = history.size(); i <= historySize; ++i) {
                CdoSnapshot nthCommit = history.get(historySize - i);
                String commitID = commitIDAsString(nthCommit);
                for (final String propertyKey : nthCommit.getChanged()) {
                    PropertyUtils.setProperty(snapshot, propertyKey, nthCommit.getPropertyValue(propertyKey));
                }
                if (commit.getId().equals(commitID)) {
                    found = true;
                    break;
        ...
        if (!found) {
            throw new InvalidCommitInformationException("Commit " + commit.getId() + " does not exist.");
        }
        return snapshot;
    ...
    public Object getSnapshot(final String objectID, final Class<?> objectClass, final String commitID) {
        ...
        return this.getSnapshot(commit);

...
public class MergeProjectBranchCommandHandler implements ICommandHandler<MergeProjectBranchCommand, ProjectBranch> {
	...
	@Override
	public ProjectBranch handle(@Valid final MergeProjectBranchCommand command) {

		final ProjectBranch origin = command.getOrigin();

		if (origin == null) {
			throw new MissingProjectBranchInformationException("No origin branch defined");
		}

		final ProjectBranch destination = command.getDestination();
	...
	private RequirementsTree getRequirementsTree(final ProjectBranch branch) {
	...
	private void mergeRequirements(final ProjectBranch origin, final ProjectBranch destination) {

		final Collection<Requirement> originRequirements = requirementDAO.findByProjectBranchId(origin.getId());
		Collection<Requirement> destinationRequirements = requirementDAO.findByProjectBranchId(destination.getId());

		final RequirementsTree originTree = getRequirementsTree(origin);
		final RequirementsTree destinationTree = getRequirementsTree(destination);

		for(final Requirement req : originRequirements) {

			final Requirement destinationReq = RequirementUtils.findByKey(destinationRequirements, req.getKey());

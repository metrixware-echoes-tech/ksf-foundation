...
import fr.echoes.labs.foremanclient.Module;
import fr.echoes.labs.foremanclient.Modules;
import fr.echoes.labs.foremanclient.Parameter;
import fr.echoes.labs.foremanclient.PuppetClass;
import fr.echoes.labs.foremanclient.UnusedIpGetter;
...
public class ForemanService implements IForemanService {
     ...
     * @return
     */
     private Role findRole(final IForemanApi api, final String rolename) {
...
//            }
     ...
     private void createHostGroup(IForemanApi api, String projectName) {
    	 ...
    	 final ForemanConfigurationBean config = this.configurationService.getConfigurationBean();
          ...
          hostGroup.subnet_id = config.getHostgroupSubnetId();
          hostGroup.realm_id = config.getHostgroupRealmId();
          hostGroup.architecture_id = config.getArchitectureId();
          hostGroup.operatingsystem_id = config.getHostgroupOperatingsystemId();
          hostGroup.ptable_id = config.getHostgroupPtableId();
          hostGroup.medium_id = config.getHostgroupMediumId();
          hostGroup.domain_id = config.getDomainId();
	...
	private void waitVmShutdown(IForemanApi api, final Host host) {
     ...
     private String getUnusedIp() {
    	final String unusedIpScriptPath = this.configurationService.getConfigurationBean().getUnusedIpScript();
